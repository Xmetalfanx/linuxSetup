# Main Tasks

| Menu Tasks                  |   | Fedora 29 | Fedora 30  | Opensuse Leap 42.3 | Opensuse 15 | Opensuse Leap 15.1 | OpenSuse TW |
|-----------------------------|---|-----------|------------|--------------------|-------------|--------------------|-------------|
| Update System               |   | Pass      | Pass       | Not Tested         | Pass        | Not Tested         | Pass        |
| Install third party repos\* |   | Pass      | Pass       | Not Tested         | Pass        | Not Tested         | Fail        |
| Install Codecs              |   | Pass      | Not Tested | Not Tested         | Pass        | Not Tested         | Not Tested  |

# Software Menus

## Install/Update for the specific apps

| Menu Task                |   | Fedora 29  | Fedora 30  | Opensuse Leap 42.3 | OpensuseLeap 15 | OpensuseLeap 15.1 | Opensuse TW |
|--------------------------|---|------------|------------|--------------------|-----------------|-------------------|-------------|
| Add Common Software      |   | Pass\*\*   | Pass       | Not Tested         | Pass\*\*        | Not Tested        | Pass \*\*   |
| Etcher                   |   | Pass       | Not Tested | Not Tested         | Not Tested      | Not Tested        | Not Tested  |
| Wine and/or PlayonLinux  |   | Not Tested | Pass       | Not Tested         | Not Tested      | Not Tested        | Not Tested  |
| Build Essential Packages |   | Pass       | Pass       | Not Tested         | Pass            | Not Tested        | Pass        |

\*\*Warning about package downgrades due to ffmpeg ... I will let the user decide what to do

## Multimedia Apps
<<<<<<< HEAD
| Menu Task             |   | Fedora 29  | Fedora 30                     | Opensuse Leap 42.3 | OpensuseLeap 15   | OpensuseLeap 15.1 | Opensuse TW |
|-----------------------|---|------------|-------------------------------|--------------------|-------------------|-------------------|-------------|
| Tiny Media Manager    |   | Pass       | Fail - Downloads, Doesn't run | Not Tested         | Pass              | Pass              | Pass        |
| makeMKV               |   | Not Tested | Not Tested                    | Not Tested         | Not Tested        | Not Tested        | Not Tested  |
| DeadBeef Media Player |   | Fail       | Fail                          | Not Tested         | Fail - Dep issues | Not Tested        | Pass        |
| XnViewMP              |   | Pass       | Not Tested                    | Not Tested         | Pass              | Not Tested        | Pass        |

<<<<<<< HEAD
| Menu Task             |     | Fedora 29 | Fedora 30  | Opensuse Leap 42.3 | OpensuseLeap 15   | OpensuseLeap 15.1 | Opensuse TW |
| :-------------------- | :-- | :-------- | :--------- | :----------------- | :---------------- | :---------------- | :---------- |
| Tiny Media Manager    |     | Pass      | Not Tested | Not Tested         | Pass              | Not Tested        | Pass        |
| makeMKV               |     | RETEST    | Not Tested | Not Tested         | RETEST            | Not Tested        | Not Tested  |
| DeadBeef Media Player |     | Fail      | Not Tested | Not Tested         | Fail - Dep issues | Not Tested        | Pass        |
| XnViewMP              |     | Pass      | Not Tested | Not Tested         | Pass              | Not Tested        | Pass        |
=======

| Menu Task             |   | Fedora 29 | Fedora 30                     | Opensuse Leap 42.3 | OpensuseLeap 15   | OpensuseLeap 15.1 | Opensuse TW |
|-----------------------|---|-----------|-------------------------------|--------------------|-------------------|-------------------|-------------|
| Tiny Media Manager    |   | Pass      | Fail - Downloads, Doesn't run | Not Tested         | Pass              | Not Tested        | Pass        |
| makeMKV               |   | RETEST    | Not Tested                    | Not Tested         | RETEST            | Not Tested        | Not Tested  |
| DeadBeef Media Player |   | Fail      | Fail                          | Not Tested         | Fail - Dep issues | Not Tested        | Pass        |
| XnViewMP              |   | Pass      | Not Tested                    | Not Tested         | Pass              | Not Tested        | Pass        |
>>>>>>> 96497fe... Various Tests and Fixes/Tweaks


<<<<<<< HEAD
* TinyMM still has the issue of reinstalling java even if it's detected (detecting needs to be fixed)... other than that ... it works
=======
- TMM issue on Fedora - 99.99999% sure it's not installing java first which TMM needs 

---
>>>>>>> 0a8d91f... Simplifying MakeMKV function
=======
* TinyMM not detecting Java first

---
>>>>>>> 96497fe... Various Tests and Fixes/Tweaks

## Productivity/Office Apps

| Menu Task   |   | Fedora 29 | Fedora 30 | Opensuse Leap 42.3 | OpensuseLeap 15 | OpensuseLeap 15.1 | Opensuse TW |
|-------------|---|-----------|-----------|--------------------|-----------------|-------------------|-------------|
| Atom Editor |   | Fail      | Pass      | Not Tested         | Pass            | Not Tested        | Fail        |
| vscode      |   | Pass      | Pass      | Not Tested         | Pass            | Not Tested        | Pass        |


--- 

## Web Browser Installation
<<<<<<< HEAD

<<<<<<< HEAD
| Menu Items    |     | Fedora 29    | Fedora 30  | OpenSuse Leap 42.3 | Opensuse Leap 15 | OpenSuse Leap 15.1 | OpenSuse Tw |
| :------------ | :-- | :----------- | ---------- | ------------------ | ---------------- | ------------------ | ----------- |
| Google Chrome |     | Fail         | Not Tested | Not Tested         | Not Tested       | Not Tested         | Pass        |
| Chromium      |     | Pass         | Not Tested | Not Tested         | Pass             | Not Tested         | Fail        |
| Firefox       |     | Preinstalled | Not Tested | Not Tested         | Fail\*\*\*\*     | Not Tested         | Not Tested  |
| Vivaldi       |     | Fail         | Not Tested | Not Tested         | Not Tested       | Not Tested         | Pass        |
=======
| Menu Items    |   | Fedora 29    | Fedora 30 | OpenSuse Leap 42.3 | Opensuse Leap 15 | OpenSuse Leap 15.1 | OpenSuse Tw |
|---------------|---|--------------|-----------|--------------------|------------------|--------------------|-------------|
| Google Chrome |   | Fail         | Pass      | Not Tested         | Not Tested       | Pass               | Pass        |
| Chromium      |   | Pass         | Pass      | Not Tested         | Pass             | Pass               | Fail        |
| Firefox       |   | Preinstalled | Pass      | Not Tested         | Fail             | Not ... Sure       | Not Tested  |
| Vivaldi       |   | Fail         | Pass      | Not Tested         | Not Tested       | Pass               | Pass        |
>>>>>>> 0a8d91f... Simplifying MakeMKV function
=======
| Menu Items    |   | Fedora 29    | Fedora 30 | OpenSuse Leap 42.3 | Opensuse Leap 15 | OpenSuse Leap 15.1 | OpenSuse Tw |
|---------------|---|--------------|-----------|--------------------|------------------|--------------------|-------------|
| Google Chrome |   | Fail         | Pass      | Not Tested         | Not Tested       | Not Tested         | Pass        |
| Chromium      |   | Pass         | Pass      | Not Tested         | Pass             | Not Tested         | Fail        |
| Firefox       |   | Preinstalled | Pass      | Not Tested         | Fail\*\*\*\*     | Not Tested         | Not Tested  |
| Vivaldi       |   | Fail         | Pass      | Not Tested         | Not Tested       | Not Tested         | Pass        |
>>>>>>> 96497fe... Various Tests and Fixes/Tweaks

\*\*\*\* MozillaFirefox is the package name for Opensuse, not "firefox"... until i change this it's a probable fail

# Theming Installation

<<<<<<< HEAD
<<<<<<< HEAD
| Menu Items           |     | Fedora 29 | Fedora 30  | OpenSuse Leap 42.3 | Opensuse Leap 15 | OpenSuse Leap 15.1 | OpenSuse Tw |
| :------------------- | :-- | :-------- | :--------- | :----------------- | :--------------- | :----------------- | :---------- |
| Install Adapta Theme |     | Pass \*\* | Not Tested | Not Tested         | Fail             | Not Tested         | Pass        |
| Install Arc Theme    |     | Pass      | Not Tested | Not Tested         | Fail             | Not Tested         | Pass        |
| Install Mint Theme   |     | Pass      | Not Tested | Not Tested         | Not Tested       | Not Tested         | Pass        |
| Install Numix Theme  |     | Pass      | Not Tested | Not Tested         | Pass             | Not Tested         | Pass        |
| Install Plata Theme  |     | Pass      | Not Tested | Not Tested         | Fail/not coded   | Not Tested         | Pass        |
| Install Misc Icons   |     | Pass      | Not Tested | Not Tested         | Fail             | Not Tested         | Not Tested  |
| Install Misc Themes  |     | Pass      | Not Tested | Not Tested         | Fail             | Not Tested         | Not Tested  |
| All Listed Themes    |     | Pass      | Not Tested | Not Tested         | Not Tested       | Not Tested         | Not Tested  |
=======
| Menu Items           |   | Fedora 29 | Fedora 30 | OpenSuse Leap 42.3 | Opensuse Leap 15 | OpenSuse Leap 15.1 | OpenSuse Tw |
|----------------------|---|-----------|-----------|--------------------|------------------|--------------------|-------------|
| Install Adapta Theme |   | Pass      | Pass      | Not Tested         | Fail             | Pass               | Pass        |
| Install Arc Theme    |   | Pass      | Pass      | Not Tested         | Fail             | Fail               | Pass        |
| Install Mint Theme   |   | Pass      | Pass      | Not Tested         | Not Tested       | Not Tested         | Pass        |
| Install Numix Theme  |   | Pass      | Pass      | Not Tested         | Pass             | Not Tested         | Pass        |
| Install Plata Theme  |   | Pass      | Pass      | Not Tested         | Fail/not coded   | Not Tested         | Pass        |
| Install Misc Icons   |   | Pass      | Pass      | Not Tested         | Not Coded        | Not Coded          | Not Tested  |
| Install Misc Themes  |   | Pass      | Fail      | Not Tested         | Fail             | Pass               | Not Tested  |
| All Listed Themes    |   | Pass      | Fail      | Not Tested         | Not Tested       | Not Tested         | Not Tested  |


>>>>>>> 0a8d91f... Simplifying MakeMKV function
=======
| Menu Items           |   | Fedora 29 | Fedora 30 | OpenSuse Leap 42.3 | Opensuse Leap 15 | OpenSuse Leap 15.1 | OpenSuse Tw |
|----------------------|---|-----------|-----------|--------------------|------------------|--------------------|-------------|
| Install Adapta Theme |   | Pass      | Pass      | Not Tested         | Fail             | Not Tested         | Pass        |
| Install Arc Theme    |   | Pass      | Pass      | Not Tested         | Fail             | Not Tested         | Pass        |
| Install Mint Theme   |   | Pass      | Pass      | Not Tested         | Not Tested       | Not Tested         | Pass        |
| Install Numix Theme  |   | Pass      | Pass      | Not Tested         | Pass             | Not Tested         | Pass        |
| Install Plata Theme  |   | Pass      | Pass      | Not Tested         | Fail/not coded   | Not Tested         | Pass        |
| Install Misc Icons   |   | Pass      | Pass      | Not Tested         | Fail             | Not Tested         | Not Tested  |
| Install Misc Themes  |   | Pass      | Fail      | Not Tested         | Fail             | Not Tested         | Not Tested  |
| All Listed Themes    |   | Pass      | Fail      | Not Tested         | Not Tested       | Not Tested         | Not Tested  |
>>>>>>> 96497fe... Various Tests and Fixes/Tweaks

---

- Third Party Repo Info

  - Fedora

    - Both RPM Fusion Repos

  - OpenSuse

    - Pacman Repos
